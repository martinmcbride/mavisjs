...
mavis.Axes.prototype.toPixel = function(val) {
    return new mavis.Vector(this.xAxis.toPixel(val.x), this.yAxis.prange - this.yAxis.toPixel(val.y));
}
mavis.Axes.prototype.toVal = function(pixel) {
    return new mavis.Vector(this.xAxis.toVal(pixel.x), this.yAxis.toVal(this.yAxis.prange - pixel.y));
}
...
mavis.Axes.prototype.draw = function(ctx) {
    ...
    var origin = this.toPixel(new mavis.Vector(0, 0));
    ...
    for (var i = 0; i < this.xAxis.divs.length; i++) {
        if (this.xAxis.divs[i]) {
            var pos = this.toPixel(new mavis.Vector(this.xAxis.divs[i], 0));
    ...
    for (var i = 0; i < this.yAxis.divs.length; i++) {
        if (this.yAxis.divs[i]) {
            var pos = this.toPixel(new mavis.Vector(0, this.yAxis.divs[i]));
...
//Create default axes with pixel size px by py
mavis.defaultAxes = function(px, py) {
	var xAxis = new mavis.Axis({prange: px, start: 0, range: px});
	var yAxis = new mavis.Axis({prange: py, start: 0, range: py});
	var axes = new mavis.Axes(xAxis, yAxis);
	return axes;
}
